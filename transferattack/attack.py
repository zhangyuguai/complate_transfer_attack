...
class Attack(object):
    ...
    def load_model(self, model_name):
        ...
        def load_single_model(model_name):
            if model_name in models.__dict__.keys():
                ...
                # checkpoint_path = './checkpoints/'+model_name+'.bin'
                # if model_name in tf_in1k:
                #     model_name+='.tf_in1k'
                # elif model_name in tf_ens_adv_in1k:
                #     model_name+='.tf_ens_adv_in1k'
    ...
    def forward(self, data, label, **kwargs):
        ...
        for _ in tqdm(range(self.epoch), desc=f"Attack: {self.attack} attack_model:  {self.model_name}"):
            # Obtain the output
            # 获取显著图 (可以根据需求在别处使用, 例如可视化/加权/混合等)
