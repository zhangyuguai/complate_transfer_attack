...
class FIA(MIFGSM):
    ...
    Example script:
        python main.py --input_dir ./path/to/data --output_dir adv_data/fia/resnet18 --attack fia --model=resnet18
        python main.py --input_dir ./path/to/data --output_dir adv_data/fia/resnet18 --eval
    """
    ...
    def drop(self,data):
        ...
        x_drop = x_drop * Mask
        return x_drop
    def get_more(self,x, grad, **kwargs):
    ...
    def forward(self, data, label, **kwargs):
        ...
        for _ in range(self.epoch):
            ...
            #在梯度附近采样多个样本进行多样的梯度聚合。
            logits_ = self.get_logits(self.get_more(data+delta, grad=grad))
            #扩充label的形状跟logits_一致
            label_ = label.repeat(self.num_sample)
            # Calculate the loss
            loss_ = self.get_loss(logits_, label_)

            # Calculate the gradients
            grad_ = self.get_grad(loss_, delta)
            #进行多样梯度的聚合，归一化
            grad_temp=grad+0.3*grad_
            # Update adversarial perturbation
            delta = self.update_delta(delta, data,-grad_temp, self.alpha)
