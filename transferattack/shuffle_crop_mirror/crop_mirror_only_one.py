...
class BSR(MIFGSM):
    """
    高效版BSR (Block Shuffle with combined batch processing)对抗攻击
    
    继承自MIFGSM攻击方法，结合镜像和裁剪两种增强的损失
    特点:
    1. 只进行块混洗，不进行旋转
    2. 将三种变换结果(打乱、镜像、裁剪)合并为一个批次进行前向传播
    3. 支持水平镜像、垂直镜像和双向镜像
    ...
    def __init__(
        ...
        mirror_type='random',  # 镜像类型：水平/垂直/双向/随机
        mirror_loss_weight=1,  # 镜像样本损失权重
        scale_range=(0.4, 0.8),  # 裁剪缩放范围
        crop_prob=1.0,  # 块裁剪概率
        crop_loss_weight=1,  # 裁剪样本损失权重
        di_prob=0.5,  # 多样性输入概率
        **kwargs
    ):
        # 初始化MIFGSM基类
        ...
        self.shuffle_weight = 1.0           # 打乱损失权重
        
        print(f"初始化高效版 {attack} 攻击: 分块数={self.num_block}, 混洗副本数={num_scale}")
        ...
        print(f"注意：已禁用旋转功能，将三种变换合并为一个批次计算")
        
        # 用于保存当前迭代中的随机选择，确保分块和镜像一致
        self.current_block_indices = {}
    
    # ...其他方法保持不变...
    # 保留你原有的 get_length, mirror_block, crop_block, process_blocks_along_dim, shuffle, diverse_input 方法
    ...
    def transform(self, x):
        ...
        # 合并三种变换为一个大批次
        combined_batch = torch.cat([shuffled_batch, mirrored_batch, cropped_batch], dim=0)
        
        return combined_batch, self.num_scale
    ...
    def forward(self, data, label, **kwargs):
        ...
        for i in tqdm(range(self.epoch), desc=f"{self.attack} on {self.model_name} combo*********************"):
            try:
                ...
                # 执行BSR变换，获取合并的批次和每种变换的样本数量
                combined_batch, samples_per_type = self.transform(x_adv)
                
                # 一次性前向传播
                logits_combined = self.model(combined_batch)
                
                # 计算标签副本
                repeated_label = label.repeat(samples_per_type * 3)  # 三种变换的标签
                
                # 分割logits为三个部分
                shuffled_size = samples_per_type
                mirrored_size = samples_per_type
                cropped_size = samples_per_type
                
                logits_shuffled = logits_combined[:shuffled_size]
                logits_mirrored = logits_combined[shuffled_size:shuffled_size+mirrored_size]
                logits_cropped = logits_combined[shuffled_size+mirrored_size:]
                
                # 取出每种变换对应的标签
                labels_shuffled = repeated_label[:shuffled_size]
                labels_mirrored = repeated_label[shuffled_size:shuffled_size+mirrored_size]
                labels_cropped = repeated_label[shuffled_size+mirrored_size:]
                
                # 分别计算三种版本的损失
                loss_shuffled = self.get_loss(logits_shuffled, labels_shuffled)
                loss_mirrored = self.get_loss(logits_mirrored, labels_mirrored)
                loss_cropped = self.get_loss(logits_cropped, labels_cropped)
